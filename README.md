...
[English](README.en.md) · **[v1.1.0](https://github.com/wudy29/engineering-bridge/releases/tag/v1.1.0) · V1 稳定版 · 本地运行 · macOS 由维护者持续实测。** 这是 V1 稳定版（`v1.1.0`），但不表示已发布到 npm。早期/社区 Windows 用户已完成只读、补丁生成、`APPLY` 与真实写入流程；`v1.1.0` 仍需外部 Windows 与多客户端验证，不等同于维护者认证 Windows 兼容。
