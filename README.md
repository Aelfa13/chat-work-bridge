...
[English](README.en.md) · **[v1.0.0-rc.2](https://github.com/wudy29/engineering-bridge/releases/tag/v1.0.0-rc.2) · V1 Release Candidate / Pre-release · 本地运行 · macOS 由维护者持续实测。** 这是 V1 Release Candidate（`v1.0.0-rc.2`），不是稳定版 `v1.0.0`，也不表示已发布到 npm。早期/社区 Windows 用户已完成只读、补丁生成、`APPLY` 与真实写入流程；`v1.0.0-rc.2` 仍需外部 Windows 与多客户端验证，不等同于维护者认证 Windows 兼容。
...
- **看不到五个工具：** 重新连接客户端，并确认其本地 STDIO MCP 配置启动了 `dist/src/mcp-stdio.js`。
