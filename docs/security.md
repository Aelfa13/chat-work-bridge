...
This document separates enforced behavior from operating assumptions for Engineering Bridge v0.2.0-alpha.3.
...
- Application requires exact `APPLY`, rechecks canonical Git root, HEAD and the clean tracked worktree/index, and validates the patch. It accepts modifications to existing tracked regular files and exact 100644 ordinary text-file additions whose target is absent from base HEAD, the index, and the worktree.
