...
- Codex thread discovery and read accept only a registered `workspace_id`, send the canonical root as `cwd`, recheck every returned thread's canonical `cwd`, and expose bounded metadata or user/assistant projections only. `run_task_from_codex_thread` reads the source without resuming it, accepts only the verified `notLoaded` source status, forces and verifies an `ephemeral` fork, and keeps the fork in one app-server worker session. The source receives no turns and is never renamed, archived, or deleted.
- Thread-worker turns are fixed to approval `never` and `sandboxPolicy: { type: "readOnly", networkAccess: false }`. The v1 path does not accept caller permission/sandbox/network overrides, does not use `readOnly.access`, `permissions`, experimental APIs, or permission-profile lifecycle calls, and closes the worker on accept, failure, interruption, protocol failure, timeout, or Bridge process loss.
...
Task results can include `review_output`, bounded command/file-change `evidence` (with explicit truncation markers when bounds apply), a real Codex `thread_id`, `source_thread_id` and `thread_mode: "ephemeral_fork"` for thread-bridge tasks, and `partial_output` from genuine interrupts. These records are review and diagnostic material, not a durable audit log or an additional write authorization mechanism. Thread-worker/task mappings remain process-local and are intentionally not recovered after restart.
